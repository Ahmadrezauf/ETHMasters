...
$L(\omega_{1:k}, \mu_{1:k}, \Sigma_{1:k}) = - \Sigma_i log \Sigma_j N(x_i | \mu_j, \Sigma_j)$, SGD could be used, but Sigma should be kept symmetric PD, which is hard to maintain\. GMM=GBC without labels. 

\subsection*{Gaussian-Mixture Bayes classifiers}
Estimate prior $P(y)$; Est cond distr for each class as a GMM:
$P(x|y) = \sum_{j=1}^{k_y} w_j^{(y)} \mathcal{N}(x; \mu_j^{(y)}, \Sigma_j^{(y)})$. Use Bayes rule to estimate the posterior. 
